...
- `handletter.woff2` — 나눔 **손편지체**(손글씨, 한글 풀셋). `--font-pet-diary` 기본(일기 본문).
- `bumsom.woff2` — 나눔 **범솜체**(손글씨, 한글 풀셋). 본문 대안.
- `MaruMinyaHangul.subset.woff2` — 12×12 픽셀 한글(한글+라틴 서브셋 ~143KB). `--font-pixel` 기본(헤더).
- `TerrarumSansBitmap.woff2` — 픽셀 비트맵(서브셋 시 비트맵 strike 가 유실되므로 풀셋 유지). 픽셀 대안.

일기 글씨체 교체는 `globals.css` 의 `--font-pet-diary` / `--font-pixel` 한 줄만 바꾸면 된다.
@font-face 의 `?v=` 쿼리는 폰 PWA 폰트 캐시 무력화용 — 폰트 교체 시 숫자를 올린다.
